...
let userNumbersOfLP: number[] = []; // Array to store user inputs
let hashTableOfLP: number[][] = []; // The hash table
...
function insertIntoHashTableLP(): void {
...
function displayHashTableLP(size: number): void {
...
function insertNextValueLP(): void {
  if (userNumbersOfLP.length === 0) return; // Exit when no more numbers are left
...
function resetHashTableLP(): void {
