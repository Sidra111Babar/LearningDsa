...
let userNumbersOfQP: number[] = []; // Array to store user inputs
let hashTableOfQP: number[][] = []; // The hash table
...
document.getElementById('LPAddBtn')!.addEventListener('click', function() {
  ...
  const InputWrapperLP = document.getElementsByClassName('InputWrapperLP')[0] as HTMLElement;
...
function insertIntoHashTableQP(): void {
...
function displayHashTableQP(size: number): void {
  const hashTableContainer = document.getElementById('hashTableContainerLP') as HTMLElement;
...
function insertNextValueQP(): void {
    if (userNumbersOfQP.length === 0) return; // Exit when no more numbers are left
...
function resetHashTableQP(): void {
